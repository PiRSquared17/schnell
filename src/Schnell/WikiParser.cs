...
namespace Schnell
{
    ...
    public sealed class WikiParser
    {
        ...
        /*
        [ Obsolete ]
        private static IEnumerable<WikiToken> __Parse(TextReader reader)
        {
            PushbackEnumerator<string> e = new PushbackEnumerator<string>(GetLines(reader).GetEnumerator());
            ...
            while (e.MoveNext())
            {
                ...
                if (line == "{{{")
                {
                    ...
                    if (sb.Length > 0)
                    {
                        yield return WikiToken.Code;
                        yield return new WikiTextToken(sb.ToString());
                        yield return WikiToken.CodeTerm;
                    }
                }
                else if (IsTable(line))
                {
                    yield return WikiToken.Table;

                    do
                    {
                        yield return WikiToken.Row;

                        foreach (Capture capture in _cellsExpression.Match(line).Groups["t"].Captures)
                        {
                            yield return WikiToken.Cell;
                            foreach (WikiToken token in ParseInlineMarkup(capture.Value.Trim()))
                                yield return token;
                            yield return WikiToken.CellTerm;
                        }

                        yield return WikiToken.RowTerm;

                        if (!e.MoveNext())
                            break;

                        line = e.Current.TrimEnd();
                    } 
                    while (IsTable(line));

                    yield return WikiToken.TableTerm;
                }
                else
                {
                    Match match;
                    ...
                    {
                        int level = match.Groups["h"].Value.Length;
                        WikiToken heading = WikiToken.Heading(level);
                        yield return heading;
                        yield return new WikiTextToken(match.Groups["t"].Value);
                        yield return heading.ToTerminator();
                        continue;
                    }
                    ...
                    if (spaces > 0)
                    {
                        ...
                        if (nonSpace == '*' || nonSpace == '#')
                        {
                            ...
                            PushbackEnumerator<string> ee = new PushbackEnumerator<string>(e);
                            ee.Push(line);

                            while (ee.MoveNext())
                            {
                                line = ee.Current.TrimEnd();
                                ...
                                if (indent > indents.Peek())
                                {
                                    WikiToken token = line.TrimStart()[0] == '#' ? WikiToken.NumberedList : WikiToken.BulletedList;
                                    yield return token;
                                    lists.Push(token);
                                    indents.Push(indent);
                                    ee.Push(line);
                                ...
                                if (indent < indents.Peek())
                                {
                                    ...
                                    {
                                        indents.Pop();
                                        yield return lists.Pop().ToTerminator();
                                    }
                                    ee.Push(line);
                                    continue;
                                }

                                yield return WikiToken.ListItem;
                                foreach (WikiToken token in ParseInlineMarkup(line.TrimStart().Substring(1).TrimStart()))
                                    yield return token;
                                yield return WikiToken.ListItemTerm;
                            }
                        ...
                        else
                        {
                            ...
                            yield return WikiToken.Quote;
                            foreach (WikiToken token in ParseInlineMarkup(sb.ToString()))
                                yield return token;
                            yield return WikiToken.QuoteTerm;
                        }
                    }
                    else
                    {
                        StringBuilder sb = new StringBuilder(line.TrimStart());
                        ...
                        yield return WikiToken.Para;
                        foreach (WikiToken token in ParseInlineMarkup(sb.ToString()))
                            yield return token;
                        yield return WikiToken.ParaTerm;
                    }
                }
            }
        }
